...
**更新** 命令系统现已实现全面的中文本地化，在保持向后兼容性的基础上提供了完整的双语支持。所有命令均支持英文和中文别名，帮助文本也提供中文版本。
...
subgraph "命令处理器"
H_Admin["admin.py<br/>管理员命令<br/>!help, !follow, !remind, !welcome"]
H_Music["music.py<br/>音乐命令<br/>!play, !skip, !pause, !resume, !stop"]
H_Volume["volume.py<br/>音量命令<br/>!volume"]
H_Chat["chat.py<br/>聊天命令<br/>!chat, !persona, !clearctx"]
H_Debug["debug.py<br/>调试命令<br/>!ping, !status"]
end
end
subgraph "应用编排"
...
**更新** 所有处理器现在支持中英文双语命令别名，确保中文用户的本地化体验。
...
### 常用命令与使用示例
**更新** 所有命令现在支持中英文双语别名，提供完整的本地化体验。

- 管理类
  - !help / !h / !帮助：查看帮助
  - !welcome / !欢迎：设置欢迎消息模板（管理员）
  - !follow / !跟随：切换跟随模式（管理员）
  - !remind / !提醒：设置一次性提醒
- 音乐类
  - !play / !p / !播放 / !点歌：搜索并播放，支持多平台链接
  - !queue / !q / !队列：查看队列
  - !np / !正在播放 / !当前：查看正在播放
  - !skip / !s / !切歌 / !下一首：投票跳过
  - !pause / !暂停：暂停播放
  - !resume / !继续：继续播放
  - !stop / !停止：停止播放（管理员）
  - !lyrics / !lrc / !歌词：查看歌词（非直链）
  - !clear / !清空：清空队列（管理员）
  - !shuffle / !随机：随机打乱
  - !repeat / !r / !重复 / !循环：切换重复模式
  - !remove / !rm / !移除 / !删除：移除指定条目
- 音量类
  - !volume / !vol / !v / !音量：查看/设置音量
- 聊天类
  - !chat / !ask / !ai / !聊天 / !问：与 AI 对话
  - !persona / !personality / !人设 / !角色：查看/切换人设
  - !clearctx / !清上下文 / !清记录：清除上下文
- 调试类
  - !ping：响应测试
  - !status / !状态：查看状态
...
## 结论
命令系统以装饰器注册为核心，结合解析器与上下文，实现了清晰的职责分离与良好的扩展性。通过模块化的处理器与统一的回复接口，系统易于维护与演进。**更新** 现已实现全面的中文本地化支持，在保持向后兼容性的基础上为中文用户提供完整的双语体验。建议在应用层补充权限校验与参数验证，进一步提升安全性与健壮性。
