...
## 项目结构
该项目采用"容器内直接进程控制"的架构：通过入口脚本直接管理虚拟显示（Xvfb）、音频（PulseAudio）、TS3 客户端与 Python 机器人主程序；Python 主程序负责命令处理、音乐队列、AI 聊天、自动化服务与可选的 Webhook HTTP 服务。
...
**更新** 架构重构：从Supervisor多进程管理转向直接进程控制

图表来源
- [entrypoint.sh:1-103](file://docker/entrypoint.sh#L1-L103)
...
- [entrypoint.sh:1-103](file://docker/entrypoint.sh#L1-L103)
- [supervisord.conf:1-47](file://docker/supervisord.conf#L1-L47)
- [init_identity.py:1-92](file://docker/ts3client/init_identity.py#L1-L92)
- [config.yaml:1-76](file://config/config.yaml#L1-L76)
...
- 入口脚本
  - 初始化运行时目录、首次运行生成 TS3 客户端身份信息、直接启动 Xvfb、PulseAudio、TS3 客户端与 Python 机器人主程序。
- 音频与显示
...
## 架构总览
容器内进程生命周期由入口脚本直接管理，确保各子进程按序启动与自动重启。Python 机器人通过 ServerQuery 与 TS3 交互，使用 FFmpeg 播放音频，结合 Netease/yt-dlp 获取音源，可选地对外暴露 Webhook HTTP 服务。
...
Entrypoint->>Pulse : 启动音频服务
Entrypoint->>Init : 加载空洞声卡并设默认设备
...
- PulseAudio 配置注入
  - 注入最小化 PulseAudio 配置，确保音频可用。

**更新** 移除Supervisor依赖，简化镜像构建
...
- 直进程管理
  - 直接启动 Xvfb、PulseAudio、TS3 客户端与 Python 机器人主程序，所有子进程的日志输出到共享卷。

**更新** 架构重构：从Supervisor转向直接进程控制
...
- 容器依赖
  - 系统层面依赖 Xvfb、PulseAudio、FFmpeg、TS3 客户端库；移除 Supervisor 依赖，直接进程管理。

**更新** 移除Supervisor依赖，简化系统依赖
...
**更新** 性能优化：简化进程管理提升启动效率

章节来源
- [default.pa:1-20](file://docker/pulseaudio/default.pa#L1-L20)
- [config.yaml:69-76](file://config/config.yaml#L69-L76)
- [docker-compose.yml:26-29](file://docker-compose.yml#L26-L29)
...
## 结论
该部署方案通过容器化与直接进程控制，实现了 TS3 机器人在无头环境下的稳定运行。结合完善的配置系统、可选的 Webhook 接入与日志轮转，满足中小规模到中等规模团队的日常使用需求。生产环境中建议进一步强化安全与可观测性建设。**更新** 架构重构显著简化了部署流程，移除了Supervisor的复杂性，提升了启动效率和维护便利性。
...
章节来源
- [config.yaml:69-76](file://config/config.yaml#L69-L76)
- [entrypoint.sh:3-103](file://docker/entrypoint.sh#L3-L103)