...
## 简介
本项目是一个基于 TeamSpeak 3 的音乐搜索与获取机器人，支持多平台音源（网易云音乐、YouTube、Bilibili、SoundCloud 等），通过 yt-dlp 提取可播放音频流，结合队列管理器实现点歌、播放、跳过、重复等完整功能；同时提供歌词查询、音量控制、FFmpeg 播放管线与 PulseAudio 输出。系统采用模块化设计，主应用编排各子系统，命令处理器对接用户交互，服务层负责搜索与 URL 提取，队列管理器负责播放调度，音频控制器负责播放生命周期与音量。
...
- YtDlpService：对 yt-dlp 的异步封装，支持多平台 URL 解析与音频信息提取。
...
- URL 提取：对于网易云歌曲，直接访问 music.163.com/song?id=... 并由 YtDlpService.extract_audio 获取可播放 URL；对于外部链接（YouTube/B站等），同样通过 yt-dlp 抽取音频信息与 URL。
...
**章节来源**
- [music.py:17-243](file://bot/core/commands/handlers/music.py#L17-L243)
- [app.py:199-246](file://bot/app/app.py#L199-L246)
