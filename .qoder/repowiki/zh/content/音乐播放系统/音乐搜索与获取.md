...
## 简介
本项目是一个基于 TeamSpeak 3 的音乐搜索与获取机器人，支持多平台音源（网易云音乐、YouTube、Bilibili、SoundCloud 等），通过 yt-dlp 服务实现统一的 URL 提取与音频下载，结合队列管理器实现点歌、播放、跳过、重复等完整功能；同时提供歌词查询、音量控制、FFmpeg 播放管线与 PulseAudio 输出。系统采用模块化设计，主应用编排各子系统，命令处理器对接用户交互，服务层负责搜索与 URL 提取，队列管理器负责播放调度，音频控制器负责播放生命周期与音量。
