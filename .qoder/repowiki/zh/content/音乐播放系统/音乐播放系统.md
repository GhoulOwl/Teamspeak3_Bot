...
## 更新摘要
**变更内容**
- **更新** 音乐下载系统：YouTube回退源被禁用以避免机器人检测问题
- **更新** 下载目录配置：从/tmp迁移到/data/cache，提升稳定性和持久性
- **修复** _play_next函数返回值问题，确保正确的播放状态反馈
- **增强** 智能回退机制：仅使用Bilibili作为替代源，简化下载流程
- **优化** 配置管理：统一的缓存目录配置和环境变量支持
...
- **更新**：YouTube回退源被禁用以避免机器人检测问题
- **更新**：下载目录从/tmp迁移到/data/cache，提升稳定性和持久性
- **修复**：_play_next函数返回值问题，确保正确的播放状态反馈
- **增强**：简化的智能回退机制，仅使用Bilibili作为替代源
...
- **更新**：YouTube回退源被禁用，仅使用Bilibili作为替代源
- **更新**：下载目录配置从/tmp迁移到/data/cache，提升稳定性和持久性
- **修复**：_play_next函数返回值问题，确保正确的播放状态反馈
...
## 架构总览
系统以 BotApplication 为中心，将音频控制器、队列管理、网易云客户端等模块组合。命令处理器接收 TS3 文本消息，解析后调用相应服务；音频控制器负责与 FFmpeg/PulseAudio 交互；yt-dlp 负责多平台 URL 抽取和本地音频下载；队列管理负责播放顺序与用户交互。**更新的智能回退机制**通过禁用YouTube回退源，仅使用Bilibili作为替代源，避免机器人检测问题，同时通过统一的/data/cache目录提升下载系统的稳定性和持久性。
...
AC-->>CMD : 回调通知
CMD->>Q : 下一首/跳过
Note over CMD : YouTube回退源已禁用，仅使用Bilibili
Note over CMD : 下载目录统一为/data/cache
```
...
### 智能回退机制更新
**更新**：YouTube回退源被禁用以避免机器人检测问题，仅使用Bilibili作为替代源

- **回退策略变更**：
  - YouTube搜索：ytsearch1:歌曲名（**已禁用**）
  - B站搜索：bilisearch1:歌曲名（**启用**）
  - 时长验证：过滤时长小于30秒的音频
  - 成功后自动使用下载的音频文件

- **机器人检测规避**：
  - YouTube的反爬虫机制可能导致IP被封禁
  - 禁用YouTube回退源避免机器人检测风险
  - 仅使用Bilibili作为替代源，降低检测概率
...
```mermaid
flowchart TD
Start(["回退下载"]) --> CheckNetease["检查网易云下载"]
CheckNetease --> |失败| DisableYouTube["YouTube回退源已禁用"]
DisableYouTube --> EnableBilibili["启用B站回退源"]
EnableBilibili --> SearchBilibili["B站搜索: bilisearch1:歌曲名"]
SearchBilibili --> CheckDuration{"时长>=30秒?"}
CheckDuration --> |是| Download["下载音频文件"]
CheckDuration --> |否| Fail["返回None"]
Download --> Success["返回DownloadedAudio"]
Fail --> End(["结束"])
Success --> End
```

**图表来源**
- [music.py:210-238](file://bot/core/commands/handlers/music.py#L210-L238)
...
- **目录变更**：
  - 旧目录：/tmp（临时目录，可能被系统清理）
  - 新目录：/data/cache（持久化目录，容器重启后数据保留）

- **配置方式**：
  - 环境变量：YTDLP_CACHE_DIR
  - 配置文件：config/config.yaml中的cache_dir
  - 代码默认：/data/cache
...
+ get_cache_dir() str
+ validate_cache_space() bool
+ cleanup_old_files() void
}
class YtDlpService {
- out_dir : str
- outtmpl : str
+ download_audio(url, cache_dir) DownloadedAudio
+ _download_sync(url, cache_dir) void
}
class EnvironmentConfig {
-YTDLP_CACHE_DIR : str = "/data/cache"
+get_cache_dir_from_env() str
}
CacheConfig --> YtDlpService : "配置"
YtDlpService --> EnvironmentConfig : "读取环境变量"
```

**图表来源**
- [config.py:52-54](file://bot/config.py#L52-L54)
- [ytdlp.py:113](file://bot/services/netease/ytdlp.py#L113)

**章节来源**
- [config.py:52-54](file://bot/config.py#L52-L54)
- [ytdlp.py:113](file://bot/services/netease/ytdlp.py#L113)
- [config.yaml:19](file://config/config.yaml#L19)
...
- **修复内容**：
  - 明确返回类型：async def _play_next(app: BotApplication) -> bool
  - 统一返回值：成功时返回True，失败时返回False
  - 递归调用时正确传递返回值
...
```mermaid
flowchart TD
PlayNext["_play_next函数"] --> CheckEntry{"检查队列条目"}
CheckEntry --> |无条目| EmptyQueue["返回False<br/>队列已空"]
CheckEntry --> |有条目| ProcessEntry["处理队列条目"]
ProcessEntry --> URLCheck{"是否为URL播放?"}
URLCheck --> |是| DownloadURL["下载URL音频"]
URLCheck --> |否| DownloadNetease["下载网易云音频"]
DownloadURL --> URLSuccess{"下载成功?"}
DownloadURL --> |否| SkipEntry["跳过条目"]
SkipEntry --> RecursiveCall["递归调用_play_next"]
URLSuccess --> |是| PlayAudio["播放音频"]
URLSuccess --> |否| RecursiveCall
DownloadNetease --> NetEaseSuccess{"下载成功?"}
DownloadNetease --> |否| TryFallback["尝试回退下载"]
TryFallback --> FallbackSuccess{"回退成功?"}
FallbackSuccess --> |是| PlayAudio
FallbackSuccess --> |否| SkipEntry
PlayAudio --> SuccessReturn["返回True"]
RecursiveCall --> Result["返回递归结果"]
EmptyQueue --> End(["结束"])
SuccessReturn --> End
Result --> End
```
...
**图表来源**
- [music.py:241-306](file://bot/core/commands/handlers/music.py#L241-L306)
...
- 抽取策略：
  - 优先直链，否则选择最佳音频格式
  - 从多级格式中挑选最高质量音频
  - **更新** 智能回退：当网易云下载失败时，自动尝试Bilibili（YouTube已禁用）
  - **更新** B站搜索：bilisearch1:歌曲名
  - **更新** 时长验证：过滤时长过短的音频文件
  - **更新** Cookie文件支持：增强认证能力
...
- **核心功能**：
  ...
  - 本地音频下载：将在线音频下载到本地缓存目录
  - 多平台支持：网易云音乐、YouTube、Bilibili、SoundCloud 等
  - 线程池异步：避免阻塞事件循环
  - **更新** 智能回退：仅Bilibili作为网易云的替代源（YouTube已禁用）
  - **更新** 音频质量验证：使用 ffprobe 验证音频文件完整性
  - **更新** Cookie文件支持：增强认证能力
...
TryFallback --> Bilibili["B站搜索"]
Bilibili --> CheckBL{"下载成功?"}
CheckBL --> |是| Return["返回DownloadedAudio"]
CheckBL --> |否| Fail["返回None"]
BuildOpts --> CreateDir["创建缓存目录<br/>/data/cache"]
...
- 内部模块耦合：
  ...
  - **更新** 智能回退机制仅使用Bilibili，YouTube已禁用
  - **更新** 下载目录配置从/tmp迁移到/data/cache
  - **修复** _play_next函数返回值问题
  - 命令注册器支持中英文命令别名
  - **新增** 队列管理支持 URL 播放
...
- **更新** 智能回退机制：仅使用Bilibili搜索，避免YouTube机器人检测风险，减少不必要的网络请求
- **更新** 下载目录性能：/data/cache目录提供更好的文件系统性能和持久性
- **修复** _play_next函数性能：正确的返回值处理避免重复播放和资源浪费
...
## 结论
本系统通过清晰的模块划分与异步架构实现了稳定可靠的音乐播放能力。**更新的智能回退机制**通过禁用YouTube回退源，仅使用Bilibili作为替代源，有效避免了机器人检测问题，同时提升了系统的稳定性和可靠性。**更新的下载目录配置**从/tmp迁移到/data/cache，提供了更好的持久性和性能。**修复的_play_next函数**确保了正确的播放状态反馈，改善了自动播放和错误处理逻辑。FFmpeg 与 PulseAudio 的结合提供了跨平台的音频输出，yt-dlp 的多平台支持扩展了音源范围，队列与命令系统增强了用户体验。建议在生产环境中关注命令别名解析性能、简化的智能回退机制的延迟、下载目录的磁盘空间管理、缓存清理策略、音频处理禁用机制的稳定性、动态音频归一化的CPU开销、音量控制平滑性以及Cookie文件的安全管理，并完善错误恢复与监控告警。
...
- 网易云：搜索上限、音频质量、**更新** Cookie文件路径
- 日志：级别、文件路径、轮转大小与备份数
- **新增** 命令别名配置：支持中英文命令别名
- **更新** 回退机制配置：仅Bilibili搜索参数
- **新增** 缓存目录配置：统一的/data/cache目录
- **新增** 动态音频归一化配置：滤镜参数设置
